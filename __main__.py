...
class Main:
    if __name__ == '__main__':
        ...
        parser = argparse.ArgumentParser(description='wco-dl downloads shows from wco.tv')
        ...
        if args.batch:
            ...
            with open(args.batch[0], 'r') as anime_list:
                for anime in anime_list:
                    print(anime.replace('\n', ''))
                    Lifter(url=anime.replace('\n', '').replace('https://wco.tv', 'https://www.wco.tv'), resolution=args.highdef, logger=logger, season=args.season,
        ...
        if args.input is None:
            ...
            Lifter(url=args.input[0].replace('https://wco.tv', 'https://www.wco.tv'), resolution=args.highdef, logger=logger, season=args.season,
                   ep_range=args.episoderange, exclude=args.exclude, output=args.output, newest=args.newest,
                   settings=settings, database=database, threads=args.threads, quiet=quiet)
